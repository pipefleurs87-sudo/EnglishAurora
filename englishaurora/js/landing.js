...
const sky1 = new Image(); sky1.src = ROOT_PREFIX + 'assets/cielo-profundo.jpg';
const sky2 = new Image(); sky2.src = ROOT_PREFIX + 'assets/cielo-dorado.jpg';
...
function wirePanelButtons(){
  ...
  if (isStudentMode && studentToken) {
    $('btn-practice').href = `${ROOT_PREFIX}aula/ejercicio.html?t=${encodeURIComponent(studentToken)}&tema=${encodeURIComponent(topicId)}`;
    $('btn-practice').innerHTML = '✎&nbsp;&nbsp;Practice (Record Score)';
    $('btn-lesson').href   = isWaveB ? `${ROOT_PREFIX}leccion/${topicId}-wave-b.html?t=${encodeURIComponent(studentToken)}` : `${ROOT_PREFIX}leccion/${topicId}.html?t=${encodeURIComponent(studentToken)}`;
    $('btn-lesson').innerHTML = isWaveB ? '▶&nbsp;&nbsp;Lesson (Wave B Masterclass)' : '▶&nbsp;&nbsp;Lesson';
    $('btn-test').href     = `${ROOT_PREFIX}evaluacion/${topicId}.html?t=${encodeURIComponent(studentToken)}`;
  } else {
...
$('btn-ignite').addEventListener('click', ()=>{
  ...
  say(`You carry <b>${esenciaTotal()} Aurora Essence</b> ✦ and <b>${n}</b> completed paths. <b>Sign up to keep your students' progress!</b> <a href="${ROOT_PREFIX}herramientas/teacher-hub.html" style="color:var(--gold); text-decoration:underline; font-weight:600; display:inline-block; margin-top:4px;">Open Teacher's Hub ↗</a>`);
});
...
function applyStudentUI() {
  ...
  if (backAulaBtn) {
    backAulaBtn.href = `${ROOT_PREFIX}aula/estudiante.html?t=${encodeURIComponent(studentToken)}`;
