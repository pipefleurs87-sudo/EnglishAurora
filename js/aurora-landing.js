...
const SKILLS = [
  { key:'speaking',   label:'Speaking',   cx:520,  cy:760,  hue:'#8FF2D2',
    desc:'Speaking: the ability to produce the language out loud — fluency, pronunciation, and real conversation.' },
  { key:'listening',  label:'Listening',  cx:960,  cy:520,  hue:'#9DF2DF',
    desc:'Listening: understanding English as it sounds — accents, speed, and native nuances.' },
  { key:'reading',    label:'Reading',    cx:1410, cy:790,  hue:'#BFE8FF',
    desc:'Reading: understanding real texts, from a simple ad to a complex article.' },
  ...
  { key:'grammar',    label:'Grammar',    cx:2320, cy:780,  hue:'#E3C8FF',
    desc:'Grammar: the invisible architecture of the language — the structures holding everything together.' },
  { key:'vocabulary', label:'Vocabulary', cx:2750, cy:560,  hue:'#FFC9D9',
    desc:'Vocabulary: words as tools — the more you have, the more worlds you describe.' },
];
...
function selectLevel(lv){
  ...
  if(lv){
    const n = TEMAS.filter(t=>t.level===lv).length;
    skillDesc.textContent = `Level ${lv}: ${n} topics lit in the sky. The rest are waiting for their turn.`;
    say(`<b>Level ${lv}</b>: ${n} clustered stars. Combine it with a skill from the carousel to find your exact topic.`);
  } else {
    skillDesc.textContent = 'Six skills. One sky. Choose a constellation or touch a star.';
  }
}
...
function selectSkill(key){
  ...
  say(`<b>${sk.label}</b>: ${sk.count} stars in this constellation. Touch one to see its sequence.`);
}
...
$('btn-just-look').addEventListener('click', e=>{ e.preventDefault(); setAurora('saludo'); $('veil').classList.remove('show');
  say('No problem. The sky is yours to look at — <b>I\'ll be here when you want to light it up</b>.'); });
...
/* ---------- arranque ---------- */
skillDesc.textContent = 'Six skills. One sky. Choose a constellation or touch a star.';
say('Hello, I\'m <b>Aurora</b> ✨ This sky is asleep... <b>approach a star</b> and watch it wake up.');
requestAnimationFrame(frame);
